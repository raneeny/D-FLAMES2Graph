...
def train_client_node(x_train,y_train,global_graph,centriod,flag,comm_round,client_name,nb_classes):
    ...
    if(flag):
        ...
        G, G_w, cluster_central = sga.aggregate_graphs(global_graph_,centriod_,threshold)
...
clients = create_clients(x_training,y_training,4)
...
for i in range(common_round):
    ...
    threshold = 0.5
    sga = ServerGraphAggregation()
    global_g, global_g_w,contriod_c = sga.aggregate_graphs(global_graphs,centriod,threshold)
    global_graph = []
    contriod_0 = []
    path_graph = f'../data/graphs/{data_name}_server_graph_round{i}.gpickle' 
    #saving graph
    with open(path_graph, 'wb') as file_graph:
        pickle.dump(global_g, file_graph)
    path_graphW = f'../data/graphs/{data_name}_server_graphW_round{i}.gpickle'
...
save_arr = []
name = '../data/results/%s_clients_graph_accuracy.csv' %(data_name)
if os.path.exists(name):
    os.remove(name)
save_arr.append("accuracy")
save_arr.append(acc_avg)
save_arr.append("BalanceACC")
save_arr.append(avg_BalanceACC_)
save_arr.append("Precision")
save_arr.append(avg_Precision_)
save_arr.append("F1")
save_arr.append(avg_F1_)
np.savetxt(name, save_arr, delimiter=",",fmt='%s')

for i in range(len(acc_clients)):
    save_arr = []
    name = '../data/results/%s_client_%s_graph_accuracy.csv' %(data_name,i)
    if os.path.exists(name):
        os.remove(name)
    save_arr.append("accuracy")
    save_arr.append(acc_clients[i])
    np.savetxt(name, save_arr, delimiter=",",fmt='%s')