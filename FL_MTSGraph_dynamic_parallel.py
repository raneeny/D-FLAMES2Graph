...
"""
def create_clients(data, labels, num_clients, initial='clients'):
    ''' Returns a dictionary with keys as clients' names and value as data shards - tuple of images and labels lists.
    Args:
        data: a list of numpy arrays of training data
        labels: a list of binarized labels for each data
    ...
    # Pair the data with labels and shuffle them
    paired_data = np.array(list(zip(data, labels)), dtype=object)
    random.shuffle(paired_data)

    # Use StratifiedShuffleSplit to ensure all clients have all labels
    sss = StratifiedShuffleSplit(n_splits=num_clients, test_size=None, train_size=1/num_clients)
    shards = [paired_data[train_index] for train_index, _ in sss.split(data, labels)]
...
def train_client(client_data,global_graph,centriod,flag,comm_round,client_name,nb_classes, gpu_id=None):
    if gpu_id is not None:
    # Set CUDA_VISIBLE_DEVICES environment variable
        os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)

    # Setup TensorFlow to only allocate required memory
    ...
    if gpus:
        try:
            ...
            print(e)
    print(f'client {client_name} is running at round {comm_round}')
    x_train = []
    y_train = []
    for(X_test, Y_test) in client_data:
        x_train.append(X_test)
        y_train.append(Y_test)
    x_train = np.array(x_train)
    y_train = np.array(y_train)
    # Assuming you have a function train_model that trains your model and returns the trained model and its metrics
    cluster_central,G_w,model,train_model = train_client_node(x_train, y_train,global_graph,centriod,flag,comm_round,client_name,nb_classes)
    results = {
        'client_name': client_name,
        'cluster_central': cluster_central,
        'G_w': G_w,
        'model': model,
...
def execute_parallel_train_clients(clients,client_names,global_graph,centriod,flag,comm_round,nb_classes):
    # Assuming `clients` is a dictionary where keys are client names and values are their respective data.
    # You may need to adjust this line based on how your clients' data is structured.
    results = Parallel(n_jobs=4)(delayed(train_client)(clients[client_name],global_graph,centriod,flag,comm_round,client_name,nb_classes, gpu_id=i%4) for client_name in client_names)
    return results
...
for r in range(repetitions):
    ...
    for i in range(common_round):
        ...
        for client in client_names:
            ...
            training_results = execute_parallel_train_clients(clients,client_names,global_graph,contriod_0,flag,i,nb_classes)
            for result in training_results:
                global_graphs.append(result['G_w'])
                centriod.append(result['cluster_central'])
                model = result['model']
                train_model = result['train_model']
            if(i == 0):
                print("traning an epoch parallet time: %s seconds ---" % (time.time() - start_time))
                logging.debug(f'traning an epoch parallet time: %s seconds at round {i} is {(time.time() - start_time)}')
        ...
        if(i == 0):
            ...
            acc_avg.append(accur)
            avg_BalanceACC_.append(balance_acc)
            avg_F1_.append(f1_sc)
            avg_Precision_.append(pres_val)
            save_graph = True
    
        elif(i >0):             
            if(0<(acc_avg[i-1]-accur) <= 0.03 or (accur>acc_avg[i-1])):
                acc_avg.append(accur)
                avg_BalanceACC_.append(balance_acc)
                avg_F1_.append(f1_sc)
                avg_Precision_.append(pres_val)
    ...
    print("total time: %s seconds ---" % (time.time() - stime))